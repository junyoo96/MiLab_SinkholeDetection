...
MATERIAL_CONCRETE_CONDUCTIVITY_MIN = 1e-12
MATERIAL_CONCRETE_CONDUCTIVITY_MAX = 1e-5
...
def generate_cavity_cylinder(water=False, water_portion=0):
    ...
    for i in range(1,to_generate_cylinder_num+1):
        ...
        if i==to_generate_cylinder_num:
            break        

        
        if i<=to_generate_cylinder_num/2:
            
            current_cylinder_radius=utility.getXAxis(0,cavity_z_determined,current_cylinder_lower_z,cavity_radius_determined)
...
def mutliple_b_scan_auto_generation(underground_object_type,iteration_index, b_scan_iteration):
    ...
    # generate file
    print(b_scan_iteration)
    textfile_content=textfile.getvalue()
    print(textfile_content)
    print(textfile_content.index("#hertzian_dipole:"))
